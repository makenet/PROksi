		...
		<?php do_action( 'bp_before_directory_activity' ); ?>
			...
			<?php if ( !is_user_logged_in() ) : ?>
				...
				<script type="text/javascript">
					jQuery(document).ready(function(){<?php do_action( 'bp_before_directory_activity' ); ?>				
						jQuery("#front-page-info").css("display", "block");
					})	
				</script>
				
				<h3>Sivuston toiminta</h3>
			...
			<div class="item-list-tabs activity-type-tabs" role="navigation">
				<ul>
					...
					<li class="selected" id="activity-all"><a href="<?php echo bp_loggedin_user_domain() . bp_get_activity_slug() . '/'; ?>" title="<?php _e( 'The public activity for everyone on this site.', 'buddypress' ); ?>"><?php printf( __( utf8_encode('All Members <span>%s</span>'), 'buddypress' ), bp_get_total_site_member_count() ); ?></a></li>
			...
			<div class="item-list-tabs no-ajax" id="subnav" role="navigation">
				<ul>
					...
					<li id="activity-filter-select" class="last">
						<label for="activity-filter-by"><?php _e( utf8_encode('Näytä'), 'buddypress' ); ?></label> 
						<select id="activity-filter-by">
							<option value="-1"><?php _e( 'Kaikki', 'buddypress' ); ?></option>
							<option value="activity_update"><?php _e( utf8_encode('Päivitykset'), 'buddypress' ); ?></option>
							...
							<?php if ( bp_is_active( 'groups' ) ) : ?>
								...
								<option value="created_group"><?php _e( utf8_encode('Uudet opinnäytetyöt'), 'buddypress' ); ?></option>
